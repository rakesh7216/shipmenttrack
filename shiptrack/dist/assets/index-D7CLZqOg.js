(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const s of a.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function e(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(n){if(n.ep)return;n.ep=!0;const a=e(n);fetch(n.href,a)}})();const G="/api";function V(){return localStorage.getItem("shiptrack_token")}async function g(t,i,e){const o=V(),n={"Content-Type":"application/json"};o&&(n.Authorization=`Bearer ${o}`);const a=await fetch(`${G}${i}`,{method:t,headers:n,body:e?JSON.stringify(e):void 0});if(!a.ok){const s=await a.json().catch(()=>({error:a.statusText}));throw new Error(s.error||"Request failed")}return a.json()}const x={login:(t,i)=>g("POST","/auth/login",{email:t,password:i}),me:()=>g("GET","/auth/me"),shipments:(t={})=>{const i=new URLSearchParams(t).toString();return g("GET",`/shipments${i?"?"+i:""}`)},shipment:t=>g("GET",`/shipments/${t}`),createShipment:t=>g("POST","/shipments",t),deleteShipment:t=>g("DELETE",`/shipments/${t}`),injectEvent:(t,i)=>g("POST",`/shipments/${t}/events`,i),carriers:()=>g("GET","/shipments/meta/carriers"),publicTrack:t=>fetch(`${G}/tracking/${t}`).then(i=>i.json()),notifPrefs:()=>g("GET","/shipments/notifications/prefs"),saveNotifPrefs:t=>g("PUT","/shipments/notifications/prefs",t),notifLog:()=>g("GET","/shipments/notifications/log"),analyticsSummary:()=>g("GET","/analytics/summary"),carrierSla:()=>g("GET","/analytics/carrier-sla"),volume:()=>g("GET","/analytics/volume"),routes:()=>g("GET","/analytics/routes"),chat:t=>g("POST","/chat",{message:t})},$=[{email:"customer@demo.com",password:"demo",role:"customer",icon:"📦",name:"Customer",desc:"Track your orders"},{email:"shipper@demo.com",password:"demo",role:"shipper",icon:"🏭",name:"Shipper",desc:"Manage all shipments"},{email:"ops@demo.com",password:"demo",role:"ops",icon:"🎛️",name:"Ops Team",desc:"Full platform access"}];async function Q(t){t.innerHTML=`
  ...
  `,document.body.appendChild(i);const e=()=>i.remove();i.querySelectorAll(".close-modal").forEach(n=>n.onclick=e);const o=i.querySelector("#add-shipment-form");o.onsubmit=async n=>{n.preventDefault();const a=new FormData(o),s={trackingNumber:a.get("trackingNumber"),carrier:a.get("carrier"),description:a.get("description"),customerEmail:a.get("customerEmail"),origin:{name:a.get("originName"),lat:parseFloat(a.get("originLat")),lng:parseFloat(a.get("originLng"))},destination:{name:a.get("destName"),lat:parseFloat(a.get("destLat")),lng:parseFloat(a.get("destLng"))},weight:a.get("weight"),dimensions:a.get("dimensions"),service:"Standard"},d=o.querySelector("#btn-submit-shipment");d.disabled=!0,d.textContent="Creating...";try{await x.createShipment(s),e(),t&&t()}catch(l){alert("Error creating shipment: "+l.message),d.disabled=!1,d.textContent="Create Shipment"}}}const Z={in_transit:"In Transit",out_for_delivery:"Out for Delivery",delivered:"Delivered",delayed:"Delayed",awaiting_customs:"Awaiting Customs",exception:"Exception"},ee={in_transit:"#60a5fa",out_for_delivery:"#34d399",delivered:"#10b981",delayed:"#fbbf24",awaiting_customs:"#a78bfa",exception:"#f87171"};function te(t){if(!t)return"—";const i=new Date(t),o=(i-new Date)/36e5;return o<0?"Past due":o<1?`~${Math.round(o*60)} min`:o<24?`~${Math.round(o)}h`:i.toLocaleDateString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function ie(t,i=!1){return`<span class="status-badge ${t} ${i&&["out_for_delivery","in_transit"].includes(t)?"pulsing":""}">
    <span class="dot"></span>${Z[t]||t}
  </span>`}function C(t){return`
    <div class="shipment-card animate-in" data-id="${t.id}" style="--status-color:${ee[t.status]||"var(--accent)"}">
      ...
      <div class="ship-card-footer">
        <div>
          ...
          <div class="eta-value">${te(t.estimatedDelivery)}</div>
        </div>
    ...
    </div>`}async function ae(t){t.innerHTML=`
    ...
    <div id="shipments-grid" class="shipments-grid">
      <div class="loading-overlay">
        ...
        </div>`;return}p.innerHTML=i.map((r,v)=>C(r).replace('animate-in"',"animate-in animate-in-delay-"+Math.min(v,3)+'"')).join(""),p.querySelectorAll(".track-btn").forEach(r=>{r.addEventListener("click",v=>{v.stopPropagation(),window.navigate(`#/tracking/${r.dataset.id}`)})}),p.querySelectorAll(".shipment-card").forEach(r=>{r.addEventListener("click",()=>window.navigate(`#/tracking/${r.dataset.id}`))}),p.querySelectorAll(".delete-btn").forEach(r=>{r.addEventListener("click",async v=>{if(v.stopPropagation(),confirm("Are you sure you want to delete this shipment?"))try{r.disabled=!0,await x.deleteShipment(r.dataset.id),e(a())}catch(u){alert("Failed to delete shipment: "+u.message),r.disabled=!1}})})}catch(r){p.innerHTML=`<div class="loading-overlay"><span style="color:var(--clr-exception)">❌ ${r.message}</span></div>`}}const o=document.getElementById("btn-add-shipment");o&&o.addEventListener("click",()=>{X(()=>e(a()))});let n;document.getElementById("search-input").addEventListener("input",l=>{clearTimeout(n),n=setTimeout(()=>s(),300)}),document.getElementById("status-filter").addEventListener("change",s),document.getElementById("carrier-filter").addEventListener("change",s);function a(){const l={},p=document.getElementById("search-input").value.trim(),r=document.getElementById("status-filter").value,v=document.getElementById("carrier-filter").value;return p&&(l.search=p),r&&(l.status=r),v&&(l.carrier=v),l}function s(){e(a())}const d=M(l=>{var y;if(l.type!=="shipment_update"&&l.type!=="position_update")return;const p=i.findIndex(h=>{var k;return h.id===((k=l.shipment)==null?void 0:k.id)});if(p===-1)return;i[p]={...i[p],...l.shipment};const r=document.querySelector(`.shipment-card[data-id="${l.shipment.id}"]`);if(!r)return;const v=document.createElement("div");v.innerHTML=C(i[p]);const u=v.firstElementChild;r.replaceWith(u),(y=u.querySelector(".track-btn"))==null||y.addEventListener("click",h=>{h.stopPropagation(),window.navigate(`#/tracking/${u.dataset.id}`)}),u.addEventListener("click",()=>window.navigate(`#/tracking/${u.dataset.id}`))});window.addEventListener("hashchange",d,{once:!0}),e()}const ne="modulepreload",se=function(t){return"/"+t},B={},oe=function(i,e,o){let n=Promise.resolve();if(e&&e.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),d=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));n=Promise.allSettled(e.map(l=>{if(l=se(l),l in B)return;B[l]=!0;const p=l.endsWith(".css"),r=p?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${r}`))return;const v=document.createElement("link");if(v.rel=p?"stylesheet":ne,p||(v.as="script"),v.crossOrigin="",v.href=l,d&&v.setAttribute("nonce",d),document.head.appendChild(v),p)return new Promise((u,y)=>{v.addEventListener("load",u),v.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${l}`)))})}))}function a(s){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=s,window.dispatchEvent(d),!d.defaultPrevented)throw s}return n.then(s=>{for(const d of s||[])d.status==="rejected"&&a(d.reason);return i().catch(a)})};function le(t,i,e){const o=document.createElement("div");o.className="modal-backdrop",o.innerHTML=`
    <div class="modal">
      <div class="modal-header">
  ...
  `,document.body.appendChild(o);const n=()=>o.remove();o.querySelectorAll(".close-modal").forEach(s=>s.onclick=n);const a=o.querySelector("#update-status-form");a.onsubmit=async s=>{s.preventDefault();const d=new FormData(a),p={code:{in_transit:"IT",out_for_delivery:"OD",delivered:"DEL",delayed:"DLY",awaiting_customs:"AC",exception:"EX"}[d.get("status")]||"IT",description:d.get("description"),location:d.get("location")},r=a.querySelector("#btn-submit-status");r.disabled=!0,r.textContent="Updating...";try{await x.injectEvent(t,p),n(),e&&e()}catch(v){alert("Error updating status: "+v.message),r.disabled=!1,r.textContent="Update Status"}}}const _={in_transit:"In Transit",out_for_delivery:"Out for Delivery",delivered:"Delivered",delayed:"Delayed",awaiting_customs:"Awaiting Customs",exception:"Exception"},re={in_transit:"🚚",out_for_delivery:"📬",delivered:"✅",delayed:"⚠️",awaiting_customs:"🛃",exception:"🚨"},de={in_transit:"🚚",out_for_delivery:"📬",delivered:"✅",delayed:"⚠️",awaiting_customs:"🛃",exception:"🚨"};function ce(t){return new Date(t).toLocaleString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function O(t){if(!t)return"—";const i=new Date(t),o=(i-new Date)/36e5;return o<0?"Past due":o<1?`~${Math.round(o*60)} min`:o<24?`~${Math.round(o*10)/10}h`:i.toLocaleDateString("en-US",{month:"short",day:"numeric"})}function N(t){return[...t].reverse().map((e,o)=>`
    <div class="timeline-item ${o===0?"timeline-latest":""}">
      <div class="timeline-dot ${e.status}">${de[e.status]||"📌"}</div>
      <div class="timeline-content">
        <div class="timeline-event">${e.description}</div>
        <div class="timeline-meta">${e.location} · ${ce(e.timestamp)}</div>
      </div>
    </div>`).join("")}function H(t){return t?String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"):""}function pe(t){return`${location.origin}${location.pathname}#/track/${t}`}async function K(t,i){var h,k,z;t.innerHTML='<div class="loading-overlay"><div class="spinner"></div><span>Loading tracking data…</span></div>';let e;try{e=await x.shipment(i)}catch(m){t.innerHTML=`<div class="loading-overlay"><span style="color:var(--clr-exception)">❌ ${m.message}</span></div>`;return}t.innerHTML=`
    <div class="page-header">
      ...
      <div style="display:flex;gap:10px;align-items:center">
        ...
        <span class="status-badge ${e.status} pulsing"><span class="dot"></span>${_[e.status]}</span>
      </div>
    </div>
      ...
      <div class="map-container">
        ...
        <div class="map-overlay-card">
          ...
          <div class="map-stat-row">
            <div class="map-stat">
              <div class="map-stat-val" id="map-eta">${O(e.estimatedDelivery)}</div>
      ...
      <div class="timeline-panel">
        <div class="timeline-header">
          ...
          <div style="margin-top:10px;padding:12px;background:var(--surface);border-radius:var(--radius);border:1px solid var(--border)">
            ...
            <div style="font-size:13px;font-weight:500">${H(e.customer.name)}</div>
            <div style="font-size:12px;color:var(--text-3)">${H(e.customer.email)}</div>
          </div>
          `:""}
        </div>
        <div class="timeline-scroll">
          <div class="timeline" id="event-timeline">${N(e.events)}</div>
        </div>
      </div>
    </div>`,(h=document.getElementById("share-btn"))==null||h.addEventListener("click",()=>{const m=pe(e.trackingNumber);navigator.clipboard.writeText(m).then(()=>{const w=document.getElementById("share-btn");w.textContent="✅ Copied!",setTimeout(()=>{w.innerHTML="🔗 Share Track"},2e3)})});const o=document.getElementById("update-status-btn");o&&o.addEventListener("click",()=>{le(i,e.status,()=>K(t,i))}),await oe(()=>import("https://unpkg.com/leaflet@1.9.4/dist/leaflet-src.esm.js"),[]).catch(()=>null);const n=window.L;if(!n){console.error("Leaflet not loaded");return}const a=n.map("tracking-map",{center:[e.currentPosition.lat,e.currentPosition.lng],zoom:10,zoomControl:!0}),s=n.latLngBounds([[e.origin.lat,e.origin.lng],[e.destination.lat,e.destination.lng],[e.currentPosition.lat,e.currentPosition.lng]]);if((k=e.routeWaypoints)!=null&&k.length&&e.routeWaypoints.forEach(m=>s.extend([m[0],m[1]])),a.fitBounds(s,{padding:[50,50]}),n.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{attribution:"© OpenStreetMap © CARTO",subdomains:"abcd",maxZoom:19}).addTo(a),(z=e.routeWaypoints)!=null&&z.length){const m=e.routeWaypoints.map(E=>[E[0],E[1]]);n.polyline(m,{color:"#5b6ef5",weight:3,opacity:.6,dashArray:"8,6"}).addTo(a);const w=m.slice(0,(e.currentWaypointIndex||0)+1);w.length>1&&n.polyline(w,{color:"#5b6ef5",weight:4,opacity:.9}).addTo(a)}const d=n.divIcon({className:"",html:'<div style="background:#1e2535;border:2px solid #5b6ef5;border-radius:50%;width:14px;height:14px;box-shadow:0 0 8px rgba(91,110,245,0.5)"></div>',iconSize:[14,14],iconAnchor:[7,7]});n.marker([e.origin.lat,e.origin.lng],{icon:d}).bindPopup(`<strong>Origin</strong><br>${e.origin.name}`).addTo(a);const l=n.divIcon({className:"",html:'<div style="background:#10b981;border:2px solid #34d399;border-radius:50%;width:16px;height:16px;box-shadow:0 0 10px rgba(52,211,153,0.6)"></div>',iconSize:[16,16],iconAnchor:[8,8]});n.marker([e.destination.lat,e.destination.lng],{icon:l}).bindPopup(`<strong>Destination</strong><br>${e.destination.name}`).addTo(a),n.circle([e.destination.lat,e.destination.lng],{radius:8e3,color:"#10b981",fillColor:"#10b981",fillOpacity:.07,weight:1.5,dashArray:"6,4"}).addTo(a);const p=n.divIcon({className:"",html:`<div style="
    ...
    ">${re[e.status]||"📦"}</div>`,iconSize:[34,34],iconAnchor:[17,17]});let r=n.marker([e.currentPosition.lat,e.currentPosition.lng],{icon:p}).bindPopup(`<strong>${e.trackingNumber}</strong><br>${_[e.status]||e.status}`).addTo(a).openPopup();const v=[{lat:e.currentPosition.lat+.15,lng:e.currentPosition.lng-.1,name:"Distribution Hub A"},{lat:e.currentPosition.lat-.1,lng:e.currentPosition.lng+.2,name:"Sorting Facility B"}],u=n.divIcon({className:"",html:'<div style="background:var(--bg-3,#1e2535);border:1px solid #374151;border-radius:6px;padding:4px 8px;font-size:10px;color:#9ca3af;white-space:nowrap">🏭</div>',iconSize:[null,null]});v.forEach(m=>n.marker([m.lat,m.lng],{icon:u}).bindPopup(m.name).addTo(a));const y=M(m=>{var w,E,D;if(!(((w=m.shipment)==null?void 0:w.id)!==i&&((E=m.shipment)==null?void 0:E.id)!==e.id)){if(m.type==="position_update"||m.type==="shipment_update"){const b=m.shipment;b.currentPosition&&(r.setLatLng([b.currentPosition.lat,b.currentPosition.lng]),a.panTo([b.currentPosition.lat,b.currentPosition.lng],{animate:!0,duration:1.5})),b.estimatedDelivery&&(document.getElementById("map-eta").textContent=O(b.estimatedDelivery)),b.remainingKm!==void 0&&(document.getElementById("map-km").textContent=b.remainingKm)}if(m.type==="shipment_update"&&((D=m.shipment)!=null&&D.events)){const b=document.getElementById("event-timeline");b&&(b.innerHTML=N(m.shipment.events))}}});window.addEventListener("hashchange",()=>{y(),a.remove()},{once:!0})}async function ve(){return window.Chart?window.Chart:new Promise((t,i)=>{const e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@4.4.2/dist/chart.umd.min.js",e.onload=()=>t(window.Chart),e.onerror=i,document.head.appendChild(e)})}const I="rgba(255,255,255,0.06)",T={responsive:!0,maintainAspectRatio:!1,plugins:{legend:{labels:{color:"#a0aec0",boxWidth:12}}}};async function ue(t){t.innerHTML=`
    <div class="page-header">
      <div>
    ...
    <div id="analytics-content" style="display:none"></div>`;try{const[i,e,o,n]=await Promise.all([ve(),x.analyticsSummary(),x.carrierSla(),x.volume()]);i.defaults.color="#a0aec0",document.getElementById("analytics-loader").style.display="none";const a=document.getElementById("analytics-content");a.style.display="block",a.innerHTML=`
      <!-- KPI Stats -->
      ...
      </div>`,new i(document.getElementById("otp-chart"),{type:"doughnut",data:{labels:["On Time","Delayed","Exception","Customs"],datasets:[{data:[e.onTime,e.delayed,e.exceptions,e.customs],backgroundColor:["#10b981","#fbbf24","#f87171","#a78bfa"],borderWidth:2,borderColor:"#0d1117"}]},options:{...T,cutout:"72%",plugins:{...T.plugins,legend:{position:"right",labels:{color:"#a0aec0",padding:16,boxWidth:12}}}}}),new i(document.getElementById("delay-chart"),{type:"bar",data:{labels:e.delayCauses.map(s=>s.cause),datasets:[{label:"Impact (%)",data:e.delayCauses.map(s=>s.percentage),backgroundColor:["#fbbf24","#a78bfa","#60a5fa","#f87171","#34d399"],borderRadius:8,borderSkipped:!1}]},options:{...T,indexAxis:"y",plugins:{...T.plugins,legend:{display:!1}},scales:{x:{grid:{color:I},ticks:{color:"#637085"}},y:{grid:{display:!1},ticks:{color:"#a0aec0"}}}}}),new i(document.getElementById("volume-chart"),{type:"line",data:{labels:n.map(s=>s.date.slice(5)),datasets:[{label:"Total",data:n.map(s=>s.shipments),borderColor:"#5b6ef5",backgroundColor:"rgba(91,110,245,0.1)",fill:!0,tension:.4,pointRadius:3},{label:"Delivered",data:n.map(s=>s.delivered),borderColor:"#10b981",backgroundColor:"rgba(16,185,129,0.08)",fill:!0,tension:.4,pointRadius:3},{label:"Delayed",data:n.map(s=>s.delayed),borderColor:"#fbbf24",tension:.4,pointRadius:3}]},options:{...T,scales:{x:{grid:{color:I},ticks:{color:"#637085"}},y:{grid:{color:I},ticks:{color:"#637085"}}}}})}catch(i){document.getElementById("analytics-loader").innerHTML=`<span style="color:var(--clr-exception)">❌ ${i.message}</span>`}}async function me(t){t.innerHTML=`
    <div class="page-header">
      <div>
  ...
  `}async function ge(t,i){t.innerHTML=`
    <div class="page-header">
      <div>
  ...
  `;try{const e=await x.publicTrack(i);document.getElementById("public-tracking-loader").style.display="none";const o=document.getElementById("public-tracking-content");o.style.display="block",o.innerHTML=`
    ...
    `}catch(e){document.getElementById("public-tracking-loader").innerHTML=`<span style="color:red">❌ ${e.message}</span>`}}async function he(t){t.innerHTML='<div class="loading-overlay"><div class="spinner"></div><span>Loading profile…</span></div>';try{const i=await x.shipments({status:"delivered"}),e=c.name.split(" ").map(a=>a[0]).join("").slice(0,2).toUpperCase(),o=i.length===0?`<div class="empty-state" style="background:var(--surface); border:1px solid var(--border);">
          <div class="empty-icon">🛒</div>
  ...
  `,document.body.appendChild(t);const i=document.getElementById("chatbot-fab"),e=document.getElementById("chatbot-window"),o=document.getElementById("chatbot-close"),n=document.getElementById("chat-form"),a=document.getElementById("chat-input"),s=document.getElementById("chat-messages");let d=!1;function l(){d=!d,d?(e.classList.remove("hidden"),i.classList.add("active"),setTimeout(()=>a.focus(),100)):(e.classList.add("hidden"),i.classList.remove("active"))}i.addEventListener("click",l),o.addEventListener("click",l);function p(u,y=!1){const h=document.createElement("div");h.className=`chat-bubble ${y?"user":"bot"}`;const k=u.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>");h.innerHTML=k,s.appendChild(h),s.scrollTop=s.scrollHeight}function r(){const u=document.createElement("div");u.id="typing-indicator",u.className="chat-bubble bot typing",u.innerHTML='<span class="dot"></span><span class="dot"></span><span class="dot"></span>',s.appendChild(u),s.scrollTop=s.scrollHeight}function v(){const u=document.getElementById("typing-indicator");u&&u.remove()}n.addEventListener("submit",async u=>{u.preventDefault();const y=a.value.trim();if(y){p(y,!0),a.value="",r();try{const h=await x.chat(y);v(),p(h.reply||"I'm sorry, I didn't understand that.")}catch{v(),p("Unable to reach the server right now. Please try again later.")}}})}let c=null,f=null,U=null;const A=new Set;function be(t,i){c=t,localStorage.setItem("shiptrack_token",i),localStorage.setItem("shiptrack_user",JSON.stringify(t))}function Y(){c=null,localStorage.removeItem("shiptrack_token"),localStorage.removeItem("shiptrack_user")}function fe(){try{const t=localStorage.getItem("shiptrack_user");t&&(c=JSON.parse(t))}catch{}}function M(t){return A.add(t),()=>A.delete(t)}function P(){if(!c||f&&f.readyState<2)return;const i=`${location.protocol==="https:"?"wss:":"ws:"}//${location.host}/ws?userId=${c.id}&role=${c.role}`;f=new WebSocket(i),f.onopen=()=>{var e;console.log("🔌 WebSocket connected"),(e=document.getElementById("ws-indicator"))==null||e.classList.add("connected"),clearTimeout(U)},f.onmessage=e=>{try{const o=JSON.parse(e.data);A.forEach(n=>n(o))}catch{}},f.onclose=()=>{var e;(e=document.getElementById("ws-indicator"))==null||e.classList.remove("connected"),U=setTimeout(P,3e3)},f.onerror=()=>f.close()}function j(){const t=document.getElementById("sidebar"),i=document.getElementById("mobile-nav"),e=document.getElementById("user-chip"),o=document.getElementById("main-content");if(!c){t.classList.add("hidden"),i&&i.classList.add("hidden"),o.classList.add("full-width");return}t.classList.remove("hidden"),i&&i.classList.remove("hidden"),o.classList.remove("full-width");const n=c.name.split(" ").map(l=>l[0]).join("").slice(0,2).toUpperCase();e.innerHTML=`
    <div class="user-avatar">${n}</div>
    <div class="user-info">
      <div class="user-name">${c.name}</div>
      <div class="user-role">${c.role}</div>
    </div>`;const a=document.querySelectorAll(".nav-ops"),s=document.querySelectorAll(".nav-shipper"),d=document.querySelectorAll(".nav-customer");a.forEach(l=>l.style.display=c.role==="ops"?"":"none"),s.forEach(l=>l.style.display=["ops","shipper"].includes(c.role)?"":"none"),d.forEach(l=>l.style.display=c.role==="customer"?"":"none")}function S(t){document.querySelectorAll(".nav-item").forEach(i=>{i.classList.toggle("active",i.dataset.page===t)})}async function L(){const t=location.hash||"#/",i=document.getElementById("app");if(t.startsWith("#/track/")){document.getElementById("sidebar").classList.add("hidden");const e=document.getElementById("mobile-nav");e&&e.classList.add("hidden"),document.getElementById("main-content").classList.add("full-width");const o=t.replace("#/track/","");i.innerHTML="",await ge(i,o);return}if(!c){j(),i.innerHTML="",await Q(i);return}if(j(),t==="#/"||t==="#/shipments")S("shipments"),i.innerHTML="",await ae(i);else if(t.startsWith("#/tracking/")){S("shipments");const e=t.replace("#/tracking/","");i.innerHTML="",await K(i,e)}else if(t==="#/analytics"){if(!["ops","shipper"].includes(c.role)){location.hash="#/shipments";return}S("analytics"),i.innerHTML="",await ue(i)}else if(t==="#/notifications")S("notifications"),i.innerHTML="",await me(i);else if(t==="#/profile"){if(c.role!=="customer"){location.hash="#/shipments";return}S("profile"),i.innerHTML="",await he(i)}else location.hash="#/shipments"}fe();P();ye();window.addEventListener("hashchange",()=>L());const J=localStorage.getItem("shiptrack_theme")||"dark";document.documentElement.setAttribute("data-theme",J);var R;(R=document.getElementById("btn-theme"))==null||R.addEventListener("click",()=>{const i=document.documentElement.getAttribute("data-theme")==="light"?"dark":"light";document.documentElement.setAttribute("data-theme",i),localStorage.setItem("shiptrack_theme",i);const e=document.getElementById("btn-theme");e&&(e.innerHTML=i==="light"?"🌙 Dark Mode":"☀️ Light Mode")});const q=document.getElementById("btn-theme");q&&(q.innerHTML=J==="light"?"🌙 Dark Mode":"☀️ Light Mode");var W;(W=document.getElementById("btn-logout"))==null||W.addEventListener("click",()=>{Y(),f&&f.close(),location.hash="#/",L()});var F;(F=document.getElementById("btn-logout-mobile"))==null||F.addEventListener("click",()=>{Y(),f&&f.close(),location.hash="#/",L()});window.navigate=t=>{location.hash=t};window.getApp=()=>({currentUser:c,onWsMessage:M,connectWs:P});L();
