(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const s of a.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function e(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(n){if(n.ep)return;n.ep=!0;const a=e(n);fetch(n.href,a)}})();const R="/api";function F(){return localStorage.getItem("shiptrack_token")}async function m(t,i,e){const o=F(),n={"Content-Type":"application/json"};o&&(n.Authorization=`Bearer ${o}`);const a=await fetch(`${R}${i}`,{method:t,headers:n,body:e?JSON.stringify(e):void 0});if(!a.ok){const s=await a.json().catch(()=>({error:a.statusText}));throw new Error(s.error||"Request failed")}return a.json()}const h={login:(t,i)=>m("POST","/auth/login",{email:t,password:i}),me:()=>m("GET","/auth/me"),shipments:(t={})=>{const i=new URLSearchParams(t).toString();return m("GET",`/shipments${i?"?"+i:""}`)},shipment:t=>m("GET",`/shipments/${t}`),createShipment:t=>m("POST","/shipments",t),deleteShipment:t=>m("DELETE",`/shipments/${t}`),injectEvent:(t,i)=>m("POST",`/shipments/${t}/events`,i),carriers:()=>m("GET","/shipments/meta/carriers"),publicTrack:t=>fetch(`${R}/tracking/${t}`).then(i=>i.json()),notifPrefs:()=>m("GET","/shipments/notifications/prefs"),saveNotifPrefs:t=>m("PUT","/shipments/notifications/prefs",t),notifLog:()=>m("GET","/shipments/notifications/log"),analyticsSummary:()=>m("GET","/analytics/summary"),carrierSla:()=>m("GET","/analytics/carrier-sla"),volume:()=>m("GET","/analytics/volume"),routes:()=>m("GET","/analytics/routes")},$=[{email:"customer@demo.com",password:"demo",role:"customer",icon:"📦",name:"Customer",desc:"Track your orders"},{email:"shipper@demo.com",password:"demo",role:"shipper",icon:"🏭",name:"Shipper",desc:"Manage all shipments"},{email:"ops@demo.com",password:"demo",role:"ops",icon:"🎛️",name:"Ops Team",desc:"Full platform access"}];async function K(t){t.innerHTML=`
    <div class="login-page">
      ...
      <div class="login-card animate-in">
        ...
        <div class="form-group">
          ...
          <input id="login-email" class="input" type="email" value="${$[0].email}" />
        </div>
        <div class="form-group">
  ...
  `,document.body.appendChild(i);const e=()=>i.remove();i.querySelectorAll(".close-modal").forEach(n=>n.onclick=e);const o=i.querySelector("#add-shipment-form");o.onsubmit=async n=>{n.preventDefault();const a=new FormData(o),s={trackingNumber:a.get("trackingNumber"),carrier:a.get("carrier"),description:a.get("description"),customerEmail:a.get("customerEmail"),origin:{name:a.get("originName"),lat:parseFloat(a.get("originLat")),lng:parseFloat(a.get("originLng"))},destination:{name:a.get("destName"),lat:parseFloat(a.get("destLat")),lng:parseFloat(a.get("destLng"))},weight:a.get("weight"),dimensions:a.get("dimensions"),service:"Standard"},l=o.querySelector("#btn-submit-shipment");l.disabled=!0,l.textContent="Creating...";try{await h.createShipment(s),e(),t&&t()}catch(c){alert("Error creating shipment: "+c.message),l.disabled=!1,l.textContent="Create Shipment"}}}const Y={in_transit:"In Transit",out_for_delivery:"Out for Delivery",delivered:"Delivered",delayed:"Delayed",awaiting_customs:"Awaiting Customs",exception:"Exception"},V={in_transit:"#60a5fa",out_for_delivery:"#34d399",delivered:"#10b981",delayed:"#fbbf24",awaiting_customs:"#a78bfa",exception:"#f87171"};function Q(t){if(!t)return"—";const i=new Date(t),o=(i-new Date)/36e5;return o<0?"Past due":o<1?`~${Math.round(o*60)} min`:o<24?`~${Math.round(o)}h`:i.toLocaleDateString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function X(t,i=!1){return`<span class="status-badge ${t} ${i&&["out_for_delivery","in_transit"].includes(t)?"pulsing":""}">
    <span class="dot"></span>${Y[t]||t}
  </span>`}function L(t){return`
    <div class="shipment-card animate-in" data-id="${t.id}" style="--status-color:${V[t.status]||"var(--accent)"}">
      ...
      <div class="ship-card-footer">
        <div>
          ...
          <div class="eta-value">${Q(t.estimatedDelivery)}</div>
        </div>
    ...
    </div>`}async function Z(t){t.innerHTML=`
    ...
    </div>`;let i=[];async function e(c={}){const v=document.getElementById("shipments-grid");try{if(i=await h.shipments(c),document.getElementById("shipment-count").textContent=`${i.length} shipment${i.length!==1?"s":""}`,i.length===0){v.innerHTML=`<div class="empty-state">
          <div class="empty-icon">📭</div>
          <div class="empty-title">No shipments found</div>
          <div class="empty-desc">Try adjusting your filters or check back later.</div>
        </div>`;return}v.innerHTML=i.map((d,p)=>L(d).replace('animate-in"',`animate-in animate-in-delay-${Math.min(p,3)}"`)).join(""),v.querySelectorAll(".track-btn").forEach(d=>{d.addEventListener("click",p=>{p.stopPropagation(),window.navigate(`#/tracking/${d.dataset.id}`)})}),v.querySelectorAll(".shipment-card").forEach(d=>{d.addEventListener("click",()=>window.navigate(`#/tracking/${d.dataset.id}`))}),v.querySelectorAll(".delete-btn").forEach(d=>{d.addEventListener("click",async p=>{if(p.stopPropagation(),confirm("Are you sure you want to delete this shipment?"))try{d.disabled=!0,await h.deleteShipment(d.dataset.id),e(a())}catch(y){alert("Failed to delete shipment: "+y.message),d.disabled=!1}})})}catch(d){v.innerHTML=`<div class="loading-overlay"><span style="color:var(--clr-exception)">❌ ${d.message}</span></div>`}}const o=document.getElementById("btn-add-shipment");o&&o.addEventListener("click",()=>{J(()=>e(a()))});let n;document.getElementById("search-input").addEventListener("input",c=>{clearTimeout(n),n=setTimeout(()=>s(),300)}),document.getElementById("status-filter").addEventListener("change",s),document.getElementById("carrier-filter").addEventListener("change",s);function a(){const c={},v=document.getElementById("search-input").value.trim(),d=document.getElementById("status-filter").value,p=document.getElementById("carrier-filter").value;return v&&(c.search=v),d&&(c.status=d),p&&(c.carrier=p),c}function s(){e(a())}const l=I(c=>{var x;if(c.type!=="shipment_update"&&c.type!=="position_update")return;const v=i.findIndex(w=>{var u;return w.id===((u=c.shipment)==null?void 0:u.id)});if(v===-1)return;i[v]={...i[v],...c.shipment};const d=document.querySelector(`.shipment-card[data-id="${c.shipment.id}"]`);if(!d)return;const p=document.createElement("div");p.innerHTML=L(i[v]);const y=p.firstElementChild;d.replaceWith(y),(x=y.querySelector(".track-btn"))==null||x.addEventListener("click",w=>{w.stopPropagation(),window.navigate(`#/tracking/${y.dataset.id}`)}),y.addEventListener("click",()=>window.navigate(`#/tracking/${y.dataset.id}`))});window.addEventListener("hashchange",l,{once:!0}),e()}const ee="modulepreload",te=function(t){return"/"+t},z={},ie=function(i,e,o){let n=Promise.resolve();if(e&&e.length>0){document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),l=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));n=Promise.allSettled(e.map(c=>{if(c=te(c),c in z)return;z[c]=!0;const v=c.endsWith(".css"),d=v?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${d}`))return;const p=document.createElement("link");if(p.rel=v?"stylesheet":ee,v||(p.as="script"),p.crossOrigin="",p.href=c,l&&p.setAttribute("nonce",l),document.head.appendChild(p),v)return new Promise((y,x)=>{p.addEventListener("load",y),p.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${c}`)))})}))}function a(s){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=s,window.dispatchEvent(l),!l.defaultPrevented)throw s}return n.then(s=>{for(const l of s||[])l.status==="rejected"&&a(l.reason);return i().catch(a)})};function ae(t,i,e){const o=document.createElement("div");o.className="modal-backdrop",o.innerHTML=`
    <div class="modal">
      <div class="modal-header">
  ...
  `,document.body.appendChild(o);const n=()=>o.remove();o.querySelectorAll(".close-modal").forEach(s=>s.onclick=n);const a=o.querySelector("#update-status-form");a.onsubmit=async s=>{s.preventDefault();const l=new FormData(a),v={code:{in_transit:"IT",out_for_delivery:"OD",delivered:"DEL",delayed:"DLY",awaiting_customs:"AC",exception:"EX"}[l.get("status")]||"IT",description:l.get("description"),location:l.get("location")},d=a.querySelector("#btn-submit-status");d.disabled=!0,d.textContent="Updating...";try{await h.injectEvent(t,v),n(),e&&e()}catch(p){alert("Error updating status: "+p.message),d.disabled=!1,d.textContent="Update Status"}}}const D={in_transit:"In Transit",out_for_delivery:"Out for Delivery",delivered:"Delivered",delayed:"Delayed",awaiting_customs:"Awaiting Customs",exception:"Exception"},ne={in_transit:"🚚",out_for_delivery:"📬",delivered:"✅",delayed:"⚠️",awaiting_customs:"🛃",exception:"🚨"},se={in_transit:"🚚",out_for_delivery:"📬",delivered:"✅",delayed:"⚠️",awaiting_customs:"🛃",exception:"🚨"};function oe(t){return new Date(t).toLocaleString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}function _(t){if(!t)return"—";const i=new Date(t),o=(i-new Date)/36e5;return o<0?"Past due":o<1?`~${Math.round(o*60)} min`:o<24?`~${Math.round(o*10)/10}h`:i.toLocaleDateString("en-US",{month:"short",day:"numeric"})}function B(t){return[...t].reverse().map((e,o)=>`
    <div class="timeline-item ${o===0?"timeline-latest":""}">
      <div class="timeline-dot ${e.status}">${se[e.status]||"📌"}</div>
      <div class="timeline-content">
        <div class="timeline-event">${e.description}</div>
        <div class="timeline-meta">${e.location} · ${oe(e.timestamp)}</div>
      </div>
    </div>`).join("")}function O(t){return t?String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"):""}function le(t){return`${location.origin}${location.pathname}#/track/${t}`}async function W(t,i){var x,w;t.innerHTML='<div class="loading-overlay"><div class="spinner"></div><span>Loading tracking data…</span></div>';let e;try{e=await h.shipment(i)}catch(u){t.innerHTML=`<div class="loading-overlay"><span style="color:var(--clr-exception)">❌ ${u.message}</span></div>`;return}t.innerHTML=`
    ...
    </div>`,(x=document.getElementById("share-btn"))==null||x.addEventListener("click",()=>{const u=le(e.trackingNumber);navigator.clipboard.writeText(u).then(()=>{const b=document.getElementById("share-btn");b.textContent="✅ Copied!",setTimeout(()=>{b.innerHTML="🔗 Share Track"},2e3)})});const o=document.getElementById("update-status-btn");o&&o.addEventListener("click",()=>{ae(i,e.status,()=>W(t,i))}),await ie(()=>import("https://unpkg.com/leaflet@1.9.4/dist/leaflet-src.esm.js"),[]).catch(()=>null);const n=window.L;if(!n){console.error("Leaflet not loaded");return}const a=n.map("tracking-map",{center:[e.currentPosition.lat,e.currentPosition.lng],zoom:10,zoomControl:!0});if(n.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{attribution:"© OpenStreetMap © CARTO",subdomains:"abcd",maxZoom:19}).addTo(a),(w=e.routeWaypoints)!=null&&w.length){const u=e.routeWaypoints.map(k=>[k[0],k[1]]);n.polyline(u,{color:"#5b6ef5",weight:3,opacity:.6,dashArray:"8,6"}).addTo(a);const b=u.slice(0,(e.currentWaypointIndex||0)+1);b.length>1&&n.polyline(b,{color:"#5b6ef5",weight:4,opacity:.9}).addTo(a)}const s=n.divIcon({className:"",html:'<div style="background:#1e2535;border:2px solid #5b6ef5;border-radius:50%;width:14px;height:14px;box-shadow:0 0 8px rgba(91,110,245,0.5)"></div>',iconSize:[14,14],iconAnchor:[7,7]});n.marker([e.origin.lat,e.origin.lng],{icon:s}).bindPopup(`<strong>Origin</strong><br>${e.origin.name}`).addTo(a);const l=n.divIcon({className:"",html:'<div style="background:#10b981;border:2px solid #34d399;border-radius:50%;width:16px;height:16px;box-shadow:0 0 10px rgba(52,211,153,0.6)"></div>',iconSize:[16,16],iconAnchor:[8,8]});n.marker([e.destination.lat,e.destination.lng],{icon:l}).bindPopup(`<strong>Destination</strong><br>${e.destination.name}`).addTo(a),n.circle([e.destination.lat,e.destination.lng],{radius:8e3,color:"#10b981",fillColor:"#10b981",fillOpacity:.07,weight:1.5,dashArray:"6,4"}).addTo(a);const c=n.divIcon({className:"",html:`<div style="
    ...
    ">${ne[e.status]||"📦"}</div>`,iconSize:[34,34],iconAnchor:[17,17]});let v=n.marker([e.currentPosition.lat,e.currentPosition.lng],{icon:c}).bindPopup(`<strong>${e.trackingNumber}</strong><br>${D[e.status]||e.status}`).addTo(a).openPopup();const d=[{lat:e.currentPosition.lat+.15,lng:e.currentPosition.lng-.1,name:"Distribution Hub A"},{lat:e.currentPosition.lat-.1,lng:e.currentPosition.lng+.2,name:"Sorting Facility B"}],p=n.divIcon({className:"",html:'<div style="background:var(--bg-3,#1e2535);border:1px solid #374151;border-radius:6px;padding:4px 8px;font-size:10px;color:#9ca3af;white-space:nowrap">🏭</div>',iconSize:[null,null]});d.forEach(u=>n.marker([u.lat,u.lng],{icon:p}).bindPopup(u.name).addTo(a));const y=I(u=>{var b,k,P;if(!(((b=u.shipment)==null?void 0:b.id)!==i&&((k=u.shipment)==null?void 0:k.id)!==e.id)){if(u.type==="position_update"||u.type==="shipment_update"){const g=u.shipment;g.currentPosition&&(v.setLatLng([g.currentPosition.lat,g.currentPosition.lng]),a.panTo([g.currentPosition.lat,g.currentPosition.lng],{animate:!0,duration:1.5})),g.estimatedDelivery&&(document.getElementById("map-eta").textContent=_(g.estimatedDelivery)),g.remainingKm!==void 0&&(document.getElementById("map-km").textContent=g.remainingKm)}if(u.type==="shipment_update"&&((P=u.shipment)!=null&&P.events)){const g=document.getElementById("event-timeline");g&&(g.innerHTML=B(u.shipment.events))}}});window.addEventListener("hashchange",()=>{y(),a.remove()},{once:!0})}async function re(){return window.Chart?window.Chart:new Promise((t,i)=>{const e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js@4.4.2/dist/chart.umd.min.js",e.onload=()=>t(window.Chart),e.onerror=i,document.head.appendChild(e)})}const T="rgba(255,255,255,0.06)",E={responsive:!0,maintainAspectRatio:!1,plugins:{legend:{labels:{color:"#a0aec0",boxWidth:12}}}};async function de(t){t.innerHTML=`
    <div class="page-header">
      <div>
    ...
    <div id="analytics-content" style="display:none"></div>`;try{const[i,e,o,n]=await Promise.all([re(),h.analyticsSummary(),h.carrierSla(),h.volume()]);i.defaults.color="#a0aec0",document.getElementById("analytics-loader").style.display="none";const a=document.getElementById("analytics-content");a.style.display="block",a.innerHTML=`
      <!-- KPI Stats -->
      ...
      </div>`,new i(document.getElementById("otp-chart"),{type:"doughnut",data:{labels:["On Time","Delayed","Exception","Customs"],datasets:[{data:[e.onTime,e.delayed,e.exceptions,e.customs],backgroundColor:["#10b981","#fbbf24","#f87171","#a78bfa"],borderWidth:2,borderColor:"#0d1117"}]},options:{...E,cutout:"72%",plugins:{...E.plugins,legend:{position:"right",labels:{color:"#a0aec0",padding:16,boxWidth:12}}}}}),new i(document.getElementById("delay-chart"),{type:"bar",data:{labels:e.delayCauses.map(s=>s.cause),datasets:[{label:"Impact (%)",data:e.delayCauses.map(s=>s.percentage),backgroundColor:["#fbbf24","#a78bfa","#60a5fa","#f87171","#34d399"],borderRadius:8,borderSkipped:!1}]},options:{...E,indexAxis:"y",plugins:{...E.plugins,legend:{display:!1}},scales:{x:{grid:{color:T},ticks:{color:"#637085"}},y:{grid:{display:!1},ticks:{color:"#a0aec0"}}}}}),new i(document.getElementById("volume-chart"),{type:"line",data:{labels:n.map(s=>s.date.slice(5)),datasets:[{label:"Total",data:n.map(s=>s.shipments),borderColor:"#5b6ef5",backgroundColor:"rgba(91,110,245,0.1)",fill:!0,tension:.4,pointRadius:3},{label:"Delivered",data:n.map(s=>s.delivered),borderColor:"#10b981",backgroundColor:"rgba(16,185,129,0.08)",fill:!0,tension:.4,pointRadius:3},{label:"Delayed",data:n.map(s=>s.delayed),borderColor:"#fbbf24",tension:.4,pointRadius:3}]},options:{...E,scales:{x:{grid:{color:T},ticks:{color:"#637085"}},y:{grid:{color:T},ticks:{color:"#637085"}}}}})}catch(i){document.getElementById("analytics-loader").innerHTML=`<span style="color:var(--clr-exception)">❌ ${i.message}</span>`}}async function ce(t){t.innerHTML=`
    <div class="page-header">
      <div>
  ...
  `}async function pe(t,i){t.innerHTML=`
    <div class="page-header">
      <div>
  ...
  `;try{const e=await h.publicTrack(i);document.getElementById("public-tracking-loader").style.display="none";const o=document.getElementById("public-tracking-content");o.style.display="block",o.innerHTML=`
      <div class="card">
    ...
    `}catch(e){document.getElementById("public-tracking-loader").innerHTML=`<span style="color:red">❌ ${e.message}</span>`}}async function ve(t){t.innerHTML='<div class="loading-overlay"><div class="spinner"></div><span>Loading profile…</span></div>';try{const i=await h.shipments({status:"delivered"}),e=r.name.split(" ").map(a=>a[0]).join("").slice(0,2).toUpperCase(),o=i.length===0?`<div class="empty-state" style="background:var(--surface); border:1px solid var(--border);">
          <div class="empty-icon">🛒</div>
          ...
          ${i.map((a,s)=>L(a).replace('animate-in"',`animate-in animate-in-delay-${Math.min(s,3)}"`)).join("")}
        </div>`;t.innerHTML=`
      <div class="page-header">
        <div>
    ...
    `;const n=t.querySelector(".shipments-grid");n&&(n.querySelectorAll(".track-btn").forEach(a=>{a.addEventListener("click",s=>{s.stopPropagation(),window.navigate(`#/tracking/${a.dataset.id}`)})}),n.querySelectorAll(".shipment-card").forEach(a=>{a.addEventListener("click",()=>window.navigate(`#/tracking/${a.dataset.id}`))}))}catch(i){t.innerHTML=`<div class="loading-overlay"><span style="color:var(--clr-exception)">❌ ${i.message}</span></div>`}}let r=null,f=null,N=null;const C=new Set;function ue(t,i){r=t,localStorage.setItem("shiptrack_token",i),localStorage.setItem("shiptrack_user",JSON.stringify(t))}function me(){r=null,localStorage.removeItem("shiptrack_token"),localStorage.removeItem("shiptrack_user")}function ge(){try{const t=localStorage.getItem("shiptrack_user");t&&(r=JSON.parse(t))}catch{}}function I(t){return C.add(t),()=>C.delete(t)}function A(){if(!r||f&&f.readyState<2)return;const i=`${location.protocol==="https:"?"wss:":"ws:"}//${location.host}/ws?userId=${r.id}&role=${r.role}`;f=new WebSocket(i),f.onopen=()=>{var e;console.log("🔌 WebSocket connected"),(e=document.getElementById("ws-indicator"))==null||e.classList.add("connected"),clearTimeout(N)},f.onmessage=e=>{try{const o=JSON.parse(e.data);C.forEach(n=>n(o))}catch{}},f.onclose=()=>{var e;(e=document.getElementById("ws-indicator"))==null||e.classList.remove("connected"),N=setTimeout(A,3e3)},f.onerror=()=>f.close()}function H(){const t=document.getElementById("sidebar"),i=document.getElementById("user-chip"),e=document.getElementById("main-content");if(!r){t.classList.add("hidden"),e.classList.add("full-width");return}t.classList.remove("hidden"),e.classList.remove("full-width");const o=r.name.split(" ").map(l=>l[0]).join("").slice(0,2).toUpperCase();i.innerHTML=`
    <div class="user-avatar">${o}</div>
    <div class="user-info">
      <div class="user-name">${r.name}</div>
      <div class="user-role">${r.role}</div>
    </div>`;const n=t.querySelectorAll(".nav-ops"),a=t.querySelectorAll(".nav-shipper"),s=t.querySelectorAll(".nav-customer");n.forEach(l=>l.style.display=r.role==="ops"?"":"none"),a.forEach(l=>l.style.display=["ops","shipper"].includes(r.role)?"":"none"),s.forEach(l=>l.style.display=r.role==="customer"?"":"none")}function S(t){document.querySelectorAll(".nav-item").forEach(i=>{i.classList.toggle("active",i.dataset.page===t)})}async function M(){const t=location.hash||"#/",i=document.getElementById("app");if(t.startsWith("#/track/")){document.getElementById("sidebar").classList.add("hidden"),document.getElementById("main-content").classList.add("full-width");const e=t.replace("#/track/","");i.innerHTML="",await pe(i,e);return}if(!r){H(),i.innerHTML="",await K(i);return}if(H(),t==="#/"||t==="#/shipments")S("shipments"),i.innerHTML="",await Z(i);else if(t.startsWith("#/tracking/")){S("shipments");const e=t.replace("#/tracking/","");i.innerHTML="",await W(i,e)}else if(t==="#/analytics"){if(!["ops","shipper"].includes(r.role)){location.hash="#/shipments";return}S("analytics"),i.innerHTML="",await de(i)}else if(t==="#/notifications")S("notifications"),i.innerHTML="",await ce(i);else if(t==="#/profile"){if(r.role!=="customer"){location.hash="#/shipments";return}S("profile"),i.innerHTML="",await ve(i)}else location.hash="#/shipments"}ge();A();window.addEventListener("hashchange",()=>M());const G=localStorage.getItem("shiptrack_theme")||"dark";document.documentElement.setAttribute("data-theme",G);var q;(q=document.getElementById("btn-theme"))==null||q.addEventListener("click",()=>{const i=document.documentElement.getAttribute("data-theme")==="light"?"dark":"light";document.documentElement.setAttribute("data-theme",i),localStorage.setItem("shiptrack_theme",i);const e=document.getElementById("btn-theme");e&&(e.innerHTML=i==="light"?"🌙 Dark Mode":"☀️ Light Mode")});const j=document.getElementById("btn-theme");j&&(j.innerHTML=G==="light"?"🌙 Dark Mode":"☀️ Light Mode");var U;(U=document.getElementById("btn-logout"))==null||U.addEventListener("click",()=>{me(),f&&f.close(),location.hash="#/",M()});window.navigate=t=>{location.hash=t};window.getApp=()=>({currentUser:r,onWsMessage:I,connectWs:A});M();
